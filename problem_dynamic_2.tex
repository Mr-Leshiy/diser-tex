...
\end{figure}
Розглядається пружне прямокутне тіло (Рис: \ref{geom_dynamic_2}), яке займає облась,
...
Для того, щоби звести задачу до одновимірної задачі у просторі трансформант, використано інтегральне перетворення Фур'є за змінною $x$ до рівнянь \eqref{lame_dynamic_2}:
\begin{equation}\label{int_trans_dynamic_2}
    \begin{pmatrix}
        u_n(y) \\
        v_n(y)
...
Після інтегрування за частинами обох рівнянь Ламе отримано наступну систему звичайних диференціальних рівняннь у просторі трансформант
...
Застосування інтегрального перетворення \eqref{int_trans_dynamic_2} до крайових умов \eqref{bound_dynamic_2} призводить до крайових умов у просторі трансформант
...
Для того щоб розв'язати задачу у простосторі трансформант, перепишмо її у векторній формі.
...
Знайдемо тепер обернену матрицю $M^{-1}(s)$, яку побудовано у наступній формі $M^{-1}(s) = \frac{\widetilde{M(s)}}{det[M(s)]}$, де $\widetilde{M(s)}$ - транспонована матриця алгебричних доповнень,
...
Враховучи це, запишемо фундаментальну матричну систему розв'язків:
\begin{align*}

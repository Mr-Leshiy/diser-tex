...
\addcontentsline{toc}{section}{\protect\numberline{}Додаток C ЗНАХОДЖЕННЯ КОЄФІЦІЄНТІВ $c_i^k$ для $\Psi_i$, $i=\overline{0,1}$, $k=\overline{0,1}$}
\section*{\centering Додаток C}\label{ap_C}
\section*{\centering ЗНАХОДЖЕННЯ КОЄФІЦІЄНТІВ $c_i^k$ для $\Psi_i$, $i=\overline{0,1}$, $k=\overline{0,1}$}
\input{apendix_c}
